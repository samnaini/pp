...
if (!empty($shipment_items)) :
    ...
    <table class="td pp-tracking_table" cellspacing="0" cellpadding="6" style="width:100%;border-collapse:collapse">
        ...
        <tr>
            <th class="td"
                style="text-align:<?php echo esc_attr($text_align); ?>;"><?php echo esc_html($TEXT_ORDER_NUMBER) ?></th>
            <?php if ($SHOW_TRACKING_NUMBER) { ?>
                <th class="td"
                    style="text-align:<?php echo esc_attr($text_align); ?>;"><?php echo esc_html($TEXT_TRACKING_NUMBER) ?></th>
            <?php } ?>
            <?php if ($SHOW_CARRIER_DETAILS) { ?>
                <th class="td"
                    style="text-align:<?php echo esc_attr($text_align); ?>;"><?php echo esc_html($TEXT_CARRIER) ?></th>
            <?php } ?>
            <th class="td"
                style="text-align:<?php echo esc_attr($text_align); ?>;"><?php echo esc_html($TEXT_STATUS) ?></th>
            <th class="td"></th>
        </tr>
        </thead>
        <tbody>
        ...
        foreach ($shipment_items as $shipment_item) {
            ...
            ?>
            <tr>
                <td class="td" style="text-align:<?php echo esc_attr($text_align); ?>;"><a
                        href="<?php echo esc_url($track_link) ?>"
                        target="_blank"><?php echo esc_html($order_number) ?></a></td>
                <?php if ($SHOW_TRACKING_NUMBER) { ?>
                    <td class="td"
                        style="text-align:<?php echo esc_attr($text_align); ?>;"><?php echo esc_html($tracking_number) ?></td>
                <?php } ?>
                <?php if ($SHOW_CARRIER_DETAILS) { ?>
                    <td class="td"
                        style="text-align:<?php echo esc_attr($text_align); ?>;"><?php echo esc_html($express_info->name ?? '') ?></td>
                <?php } ?>
                <td class="td"
                    style="text-align:<?php echo esc_attr($text_align); ?>;"><?php echo esc_html($shipment_statuses[(new ParcelPanelFunction)->parcelpanel_get_shipment_status($shipment_item->shipment_status)]['text']) ?></td>
                <td class="td" style="text-align:<?php echo esc_attr($text_align); ?>;"><a
                        href="<?php echo esc_url($track_link) ?>"
                        target="_blank"><?php echo esc_html($TEXT_TRACK) ?></a></td>
            </tr>
            <?php
        }
        ?>
