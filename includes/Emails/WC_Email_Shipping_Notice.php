...
class WC_Email_Shipping_Notice extends \WC_Email
{
     ...
     * @param int[] $tracking_ids filter tracking id
     */
    public function trigger( $order_id, $order = false, $tracking_ids = [] )
    {
        global $wpdb;
        ...
        $tracking_ids = array_unique( $tracking_ids, SORT_NUMERIC );
...
WHERE {$where_str}
SQL;
        $_shipment_items = $wpdb->get_results( $wpdb->prepare( $SQL, $query_args ) );
...
SQL;
        $tracking_item_by_id = array_column( $wpdb->get_results( $wpdb->prepare( $SQL_SELECT_TRACKINGS_BY_ID, $shipment_tracking_ids ) ), null, 'id' );
