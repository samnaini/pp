...
?>
<script>
  var pp_param = <?php echo wc_esc_json( $pp_param, true ) ?>;
...
<template id="pp-app">
  ...
  <div v-else id="pp-shipments" class="pp-wrap pp-wrap--wide">
    ...
    <modal :open="isOpenExportModal" @cancel="isOpenExportModal=false" title="Export shipments" cancel="" confirm="">
      ...
      <p class="pp-m-t-4">
          <?php echo sprintf( esc_html__( 'Learn more about %1$sCSV table headers%2$s.', 'parcelpanel' ), '<a href="https://docs.parcelpanel.com/woocommerce/article/69" target="_blank">', '</a>' )
          ?>
      </p>
      <div v-show="isExporting" class="pp-m-t-4" style="width: 100%">
        <div class="pp-m-b-1" style="display:flex">
          <div><p><?php esc_html_e( 'Exporting:', 'parcelpanel' ) ?></p></div>
          <div style="flex: 1;text-align: right;"><p>{{exportPercentage}}%</p></div>
        </div>
