...
?>
<script>
  const pp_settings_save_nonce = "<?php echo wp_create_nonce( 'pp-settings-save' ) ?>"
  const pp_courier_matching_save_nonce = "<?php echo wp_create_nonce( 'pp-courier-matching-save' ) ?>"
</script>
<template id="pp-app">
    ...
    <div class="pp-card">
      ...
      <div class="pp-card-body" style="position:relative">
        ...
        <div v-show="isShowDisabled && resultList.length" class="pp-m-t-4 pp-p-b-4" style="height: 36px;display: flex;align-items: center;border-bottom: 1px solid #F0F0F0;">
          ...
          <label v-show="count==0 && !this.isSearching"><?php echo sprintf( esc_html__( 'Showing %1$s of %2$s couriers', 'parcelpanel' )
                  , '{{resultList.length}}', '{{listTotal}}' ) ?></label>

          <label v-show="count==0 && this.isSearching"><?php echo sprintf( esc_html__( 'Showing %1$s couriers', 'parcelpanel' )
                  , '{{listTotal}}' ) ?></label>

          <label v-show="count>0"><?php echo sprintf( esc_html__( '%1$s selected', 'parcelpanel' )
                  , '{{count}}' ) ?></label>
        ...
        <div v-show="!isShowDisabled && resultList.length" class="pp-m-t-4 pp-p-b-4" style="height: 36px; display: flex;align-items: center;border-bottom: 1px solid #F0F0F0;">
          ...
          <label v-show="count==0"><?php echo sprintf( esc_html__( 'Showing %1$s couriers', 'parcelpanel' )
                  , '{{resultList.length}}' ) ?></label>
