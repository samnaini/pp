...
?>
<script>
  const pp_bind_account = "<?php echo wp_create_nonce( 'pp-bind-account' )?>"
  const pp_enable_dropshipping_nonce = "<?php echo wp_create_nonce( 'pp-enable-dropshipping-mode' )?>"

  // 判断是否已经绑定了账号
  const pp_is_empty_api_kay = "<?php echo empty( get_option( \ParcelPanel\OptionName\API_KEY, '' ) ) ?>"
  const pp_auth_key = "<?php echo wc_clean( $_GET[ 'auth_key' ] ?? '' ) ?>"
</script>
<template id="pp-app">
  ...
  <div v-else id="pp-home" class="pp-wrap">
    ...
    <div class="pp-card pp-m-t-5">
      ...
      <div class="pp-card-body">
        <p><?php echo sprintf( esc_html__( 'Your branded tracking page is ready, the URL is %1$s, add it to your store menus so your customers can track orders there.', 'parcelpanel' ),
                '<a :href="pageParam.track_page_link" target="_blank">{{pageParam.track_page_link}}</a>' )
            ?></p>
        <p class="pp-m-t-2"><?php echo sprintf( esc_html__( 'Don’t know how to do this? Please follow %1$sthis instruction%2$s.', 'parcelpanel' )
                , '<a href="https://docs.parcelpanel.com/woocommerce/article/56" target="_blank">', '</a>' )
            ?></p>
    ...
    <div class="pp-card">
      ...
      <div class="pp-card-body">
        <p>
            <?php echo sprintf( esc_html__( 'Use mapping feature to bulk import tracking number with a CSV file, or manually add one by one in the Edit order page. %1$sLearn more%2$s.', 'parcelpanel' )
                , '<a href="https://docs.parcelpanel.com/woocommerce/article/how-does-parcelpanel-work-in-woocommerce/" target="_blank">', '</a>' )
            ?>
        </p>
      ...
      <div class="pp-card-body">
        <div class="pp-m-l-4">
          <ul class="pp-m-t-0 pp-m-b-5" style="list-style: decimal">
            ...
            <li class="pp-m-t-1"><?php echo sprintf( esc_html__( 'Hide all Chinese origin easily, brings your customers an all-around brand shopping experience. %1$sLearn more%2$s.', 'parcelpanel' ), '<a href="https://docs.parcelpanel.com/woocommerce/article/61" target="_blank">', '</a>' )
                ?></li>
          </ul>
        </div>
