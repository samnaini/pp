    ...
    <div class="pp-flex-row account-container-content-box">
      <div class="left pp-card-light">
        ...
        <template v-else-if="statusRequest.FINISHED === reqStatus">
          <p v-if="isUnlimitedPlan"><?php echo sprintf( esc_html__( 'You are on the ParcelPanel %1$s plan and have %2$s cycle.', 'parcelpanel' )
                  , 'Unlimited', 'unlimited quota per month' ) ?></p>
          <p v-else><?php echo sprintf( esc_html__( 'You are on the ParcelPanel %1$s plan and have %2$s cycle. Next cycle will reset on %3$s.', 'parcelpanel' )
                  , '{{planName}}', '{{summary}}', '{{expiredDate}}' ) ?></p>
        </template>
        ...
        <template v-else-if="statusRequest.FINISHED === reqStatus">
          ...
          <div class="pp-card" v-if="isUnlimitedPlan">
            ...
            <div class="pp-card-body">
              ...
              <p style="color: #50575E;"><?php echo sprintf( esc_html__( 'Note: Remaining means you have %1$s available quota.', 'parcelpanel' )
                      , 'unlimited' ) ?></p>
            </div>
          </div>
          ...
          <div class="pp-card" v-else>
            ...
            <div class="pp-card-body">
              ...
              <p style="color: #50575E;"><?php echo sprintf( esc_html__( 'Note: Remaining means you have %1$s available quota.', 'parcelpanel' ), '{{quotaRemain}}' ) ?></p>
            </div>
          </div>
        </template>
      </div>
