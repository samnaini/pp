...
class RestApi extends \WP_REST_Controller
{
    ...
    function permission_check(\WP_REST_Request $request)
    {
        ...
        if (!$req_token && !empty($_GET['pp_token'])) {  // WPCS: CSRF ok.
            $req_token = $_GET['pp_token'];  // WPCS: CSRF ok, sanitization ok.
        }
    ...
    static function authenticate($user_id)
    {
        ...
        $headers = json_decode(json_encode((new ParcelPanelFunction())->getallheaders()), 1);
        $api_key = null;
