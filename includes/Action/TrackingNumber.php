...
class TrackingNumber
{
    ...
    function csv_importer()
    {
        ...
        $record_data = self::record_log( $import_id, basename( $file ), $total, $results[ 'succeeded_count' ], count( $results[ 'failed_msg' ] ), $results[ 'failed_msg' ] );
        $res = [
    ...
    function get_records_ajax()
    {
        ...
        foreach ( array_reverse( $record_ids, true ) as $key => $id ) {
            ...
            $record_data[ 'date' ] = date( 'Y-m-d', $record_data[ 'created_at' ] );
    ...
    static function sync_tracking()
    {
        ...
        while ( true ) {
            ...
            // sync_time：[0,2]，最多同步 3 次；每次100条
            $tracking_data = $wpdb->get_results(
                $wpdb->prepare(
                    "SELECT DISTINCT ppt.id,ppti.order_id,ppti.order_item_id,ppti.quantity,tracking_number,courier_code,fulfilled_at FROM {$TABLE_TRACKING} AS ppt INNER JOIN {$TABLE_TRACKING_ITEMS} AS ppti ON ppt.id=ppti.tracking_id WHERE sync_times BETWEEN 0 AND 2 AND ppt.id NOT IN ({$placeholder_str}) AND tracking_number<>'' ORDER BY ppti.order_id ASC LIMIT 100",
                    $not_in_ids
                )
            );
            ...
            // 整理格式：tracking_number => tracking_item
            ...
            foreach ( $tracking_data as $k => $tracking ) {
                ...
                $fulfilled_at = $tracking->fulfilled_at ? date_i18n( \DateTimeInterface::ATOM, $tracking->fulfilled_at ) : '';
                ...
                $order_item_data = $wpdb->get_results(
                    $wpdb->prepare(
                        "SELECT DISTINCT order_item_id,quantity FROM {$TABLE_TRACKING_ITEMS} WHERE tracking_id = {$tracking_id}"
                    )
                );
...
SQL;
                        $tracking_items = (array)$wpdb->get_results( $SQL_Item );
                ...
                $payloads[] = [
                    ...
                    'products'        => $productM,
                    'order'           => Orders::get_formatted_item_data( $order ),
                ];
            }
            ...
            // 更新额度信息
            (new ParcelPanelFunction)->parcelpanel_update_quota_info( $resp );

            // 处理失败的单号；非失败就是成功
            $error_tracking_numbers = (array)$resp[ 'errors' ] ?? [];


            // 处理失败的 Tracking ID
            $error_ids = [];

            foreach ( $error_tracking_numbers as $tn ) {

                $tracking_id = $tracking_data[ $tn ]->id ?? 0;

                if ( ! empty( $tracking_id ) ) {

                    $error_ids[] = $tracking_id;

                    // 剩下的属于处理成功的单号数据
                    unset( $tracking_data[ $tn ] );
                }
            }

            if ( $error_ids ) {
                // 记录同步次数
                $placeholder_str = (new ParcelPanelFunction)->parcelpanel_get_prepare_placeholder_str( $error_ids, '%d' );
                $wpdb->query( $wpdb->prepare( "UPDATE {$TABLE_TRACKING} SET `sync_times` = `sync_times` + 1 WHERE `id` IN ({$placeholder_str})", $error_ids ) );
            }
            ...
            if ( $success_ids ) {
                ...
                $wpdb->query( $wpdb->prepare( "UPDATE {$TABLE_TRACKING} SET `sync_times` = -1 WHERE `id` IN ({$placeholder_str})", $success_ids ) );
            }
        }
    ...
    static function sync_tracking_courier()
    {
        ...
        // 获取同步成功的单号数据
        $tracking_number_list = $wpdb->get_col( $wpdb->prepare( "SELECT tracking_number FROM {$TABLE_TRACKING} WHERE sync_times=-1 AND courier_code=''" ) );
        ...
        foreach ( $tracking_numbers_chunk as $tracking_number_list ) {
            ...
            foreach ( $resp as $v ) {
                ...
                $wpdb->query( $wpdb->prepare( "UPDATE {$TABLE_TRACKING} SET courier_code=%s WHERE tracking_number=%s AND courier_code=''", $courier_code, $tracking_number ) );
            }
        }
     ...
     * 计划单号同步任务
     */
    public static function schedule_tracking_sync_action( int $delay = 30 )
    {
        if ( $delay === -1 ) {
    ...
    static function tmpl_upload_modal()
    {
          ...
          <div role="document" class="components-modal__content">
            ...
            <div class="pp-modal-body">
              <div id="pp-pnl-upload">
                <div style="margin-bottom:16px"><p>Step 1: Copy the <a href="https://docs.google.com/spreadsheets/d/1NEJqC-yS0GoAkFx5jCqyVstP-tDgPvqle_hbksFWZ4g/copy" target="_blank">sample template</a> on Google Sheets(strongly recommend) or download <a href="<?php
                        echo esc_url( $import_template_file_link ) ?>" target="_blank">this CSV file</a>.</p></div>
                <div style="margin-bottom:16px"><p>Step 2: Fill in the data following the <a href="https://docs.parcelpanel.com/woocommerce/article/57" target="_blank">Import Template Instructions</a>. Tracking number that do not comply with the instructions will not be imported.</p></div>
                <div style="margin-bottom:16px"><p>Step 3: Download the file in a CSV format and upload it.</p></div>
                <div class="pp-ipt-upload-wrapper"><input type="file" id="pp-ipt-upload-file" accept=".csv" style="padding:0;height:36px;line-height:36px"></div>
                <div id="pp-upload-progress" style="width:100%;display:none">
                  <div style="margin:0 0 4px;display:flex">
                    <div><p>Uploading:</p></div>
                    <div style="flex:1 1 0;text-align:right"><p class="percent">0%</p></div>
                  </div>
                  ...
                  <div style="margin:4px 0 0"><strong>Please DO NOT close or refresh this page before it was completed.</strong></div>
                </div>
              ...
              <div id="pp-pnl-record-detail" style="display:none">
                ...
                <div class="pp-row">
                  <div class="left"><p>Import file name:</p></div>
                  <div class="right"><p class="filename"></p></div>
                </div>
                <div class="pp-row">
                  <div class="left"><p>Total tracking numbers:</p></div>
                  <div class="right"><p class="total"></p></div>
                </div>
                <div class="pp-row">
                  <div class="left"><p>Succeeded:</p></div>
                  <div class="right"><p class="succeeded"></p></div>
                </div>
                <div class="pp-row">
                  <div class="left"><p>Failed:</p></div>
                  <div class="right"><p class="failed"></p></div>
                </div>
                <div class="pp-row">
                  <div class="left"><p>Details:</p></div>
                  <div class="right">
      ...
      <script>
        const pp_upload_nonce = "<?php echo wp_create_nonce( 'pp-upload-csv' )?>"
        const pp_import_nonce = "<?php echo wp_create_nonce( 'pp-import-csv-tracking-number' )?>"
        const pp_get_history_nonce = "<?php echo wp_create_nonce( 'pp-get-import-tracking-number-records' )?>"
      </script>
        <?php
    }
}
