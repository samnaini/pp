...
class TrackingSettings
{
    ...
    const FLAG_DISPLAY_OPTIONS = [
        ...
        'b_od_nb_a_em' => 2 << 4,  // 通过订单编号和邮件
        'b_tk_nb' => 2 << 5,  // 通过运单号
        'tracking_detailed_info' => 2 << 7,  // 跟踪详情信息
    ];
    ...
    function get_settings(): array
    {
        if (empty(self::$settings_cache)) {
            ...
            self::$settings_cache = $tracking_config['data'] ?? [];
        }
     ...
     */
    private function get_estimated_delivery_time_field(array $items = []): array
    {
        $calc_from = $items['calc_from'] ?? 0;
        $e_d_t = $items['e_d_t'] ?? [10, 20];

        $enabled = (bool)($items['enabled'] ?? false);
        $bod_enabled = (bool)($items['bod_enabled'] ?? false);
        $bod_items = (array)($items['bod_items'] ?? []);
        $_bod_items = [];
        foreach ($bod_items as $v) {
            if (empty($v['to'])) {
                continue;
            }
            $_bod_items[] = [
     ...
     * Estimated delivery time
     */
    private function set_estimated_delivery_time_field(array $items = [])
    {
        $enabled = filter_var($items['enabled'] ?? '', FILTER_VALIDATE_BOOLEAN);
        $calc_from = intval($items['calc_from']);
        $calc_from = in_array($calc_from, [0, 1]) ? $calc_from : 0;
        $bod_enabled = !!($items['bod_enabled'] ?? false);
        $bod_items = $bod_enabled ? (array)($items['bod_items'] ?? []) : [];

        $e_d_t_0 = absint($items['e_d_t'][0] ?? 10);
        $e_d_t_1 = absint($items['e_d_t'][1] ?? 20);
        if ($e_d_t_1 < $e_d_t_0) {
            $e_d_t_0 = 10;
            $e_d_t_1 = 20;
        }
        $e_d_t = [$e_d_t_0, $e_d_t_1];

        $bod_items_save_data = [];
        $uniq_ship_to_map = [];
        foreach ($bod_items as $bod_item) {
            $to = $bod_item['to'] ?? '';
            if (empty($to) || array_key_exists($to, $uniq_ship_to_map)) {
                continue;
            }
            $uniq_ship_to_map[$to] = true;

            $edt_0 = absint($bod_item['edt'][0] ?? 10);
            $edt_1 = absint($bod_item['edt'][1] ?? 20);
            if ($edt_1 < $edt_0) {
                $edt_0 = 10;
                $edt_1 = 20;
            }
        ...
        $this->settings['est_dely_t'] = [
            'enabled' => $enabled,
            'calc_from' => $calc_from,
            'e_d_t' => $e_d_t,
            'bod_enabled' => $bod_enabled,
            'bod_items' => $bod_items_save_data,
        ];
    }
     ...
     * 合并设置项
     *
     * 严格参照 DEFAULT_SETTINGS 的键名
     *
     * @param int|string $setting_field 设置项字段
     * @param array $items 设置项数据
     * @param callable|null $filter_callback 自定义过滤数据回调函数；默认：wc_clean()
     */
