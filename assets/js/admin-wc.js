(($) => {
  $(() => {
    ...
    const $shop_order_screen = $('.edit-php.post-type-shop_order')
    ...
    const pp_wc_shipment_tracking_items = {
      ...
      render_add_tracking_modal: function () {
        ...
        const get_items_dom = () => {
          return _line_items
            .map((v) => (
              ...
              `<tr><td><div class="thumb">${ v.image_url ? `<img src="${ v.image_url }" alt>` : '' }</div></td><td class="column-name">${ v.name }</td><td>${ number_input_component(v) }</td></tr>`
            ))
            .join('')
        }
        ...
        let item_block_markup = !is_fulfill_disabled
          ? (
            !is_shipped_all
              ? `<table class="pp-items"><thead><tr><th class="column-items">Items</th><th>Qty.</th></tr></thead><tbody>${ get_items_dom() }</tbody></table>`
              : `<div class="fulfilled-tip">All items have been fulfilled</div>`
          )
          : ''
...
<div role="dialog" tabindex="-1" class="components-modal__frame">
  <div role="document" class="components-modal__content" style="display:flex;flex-direction:column">
    <div class="components-modal__header">
      <div class="components-modal__header-heading-container"><h1 class="components-modal__header-heading">${ title } - #${ order_number }</h1></div>
...
<div><label>${ i18n.tracking_number }<input type="text" id="pp-tk-ipt-tracking-number" value="${ tracking_number }"></label></div>

<div><label>${ i18n.courier }<select id="pp-tk-slc-courier"><option value>${ i18n.auto_matching }</option></select></label></div>

<div><label>${ i18n.mark_order_as }<select id="pp-tk-slc-mark-order-as" data-minimum-results-for-search="-1" data-placeholder="None" data-allow-clear="true"><option></option></select></label></div>

<div><label>${ i18n.date_shipped }<input type="text" id="pp-tk-ipt-date-shipped" class="date-picker-field" value="${ fulfilled_date }" placeholder="${ window.parcelpanel_admin_wc_meta_boxes.today }"></label></div>
...
<span>${ ok_text }</span>
        ...
        $('#PP-Modal-UVSNWm #pp-tk-slc-mark-order-as').selectWoo({data: window.parcelpanel_admin_wc_meta_boxes.mark_order_as_select_list, width: 'auto'})
          ...
          .on('click', '#pp-tk-btn-ok', () => {
            ...
            fetch(URL_SHIPMENT_ITEM_SAVE, {
              ...
              .then(res => {
                ...
                // 更新前端订单状态
                if (['completed', 'partial-shipped'].includes($select_mark_order_as.val())) {
  ...
  function confirm(msg, title, callback) {
    ...
    const tmpl = `<div class="components-modal__screen-overlay pp-modal" id="pp-modal-${ id }">
<div role="dialog" tabindex="-1" class="components-modal__frame">
<div role="document" class="components-modal__content" style="display: flex; flex-direction: column;">
  <div class="components-modal__header">
